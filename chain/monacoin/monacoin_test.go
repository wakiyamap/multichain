...
import (
	...
	"github.com/renproject/multichain/chain/monacoin"
...
var _ = Describe("Monacoin", func() {
	Context("when submitting transactions", func() {
		Context("when sending MONA to multiple addresses", func() {
			It("should work", func() {
				// Load private key, and assume that the associated address has
				// funds to spend. You can do this by setting MONACOIN_PK to the
				// value specified in the `./multichaindeploy/.env` file.
				pkEnv := os.Getenv("MONACOIN_PK")
				if pkEnv == "" {
					panic("MONACOIN_PK is undefined")
				}
				wif, err := btcutil.DecodeWIF(pkEnv)
				Expect(err).ToNot(HaveOccurred())

				// PKH
				pkhAddr, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeCompressed()), &monacoin.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				pkhAddrUncompressed, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeUncompressed()), &monacoin.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				...
				client := monacoin.NewClient(monacoin.DefaultClientOptions().WithHost("http://127.0.0.1:19402"))
				...
				tx, err := monacoin.NewTxBuilder(&monacoin.RegressionNetParams).BuildTx(inputs, recipients)
				Expect(err).ToNot(HaveOccurred())
				...
				// Submit the transaction to the Monacoin node. Again, this
