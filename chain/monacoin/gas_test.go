...
var _ = Describe("Gas", func() {
	Context("when estimating monacoin network fee", func() {
		It("should work", func() {
			ctx, cancel := context.WithCancel(context.Background())
			defer cancel()

			client := monacoin.NewClient(monacoin.DefaultClientOptions())

			// estimate fee to include tx within 1 block.
			fallback1 := uint64(123)
			gasEstimator1 := monacoin.NewGasEstimator(client, 1, pack.NewU256FromUint64(fallback1))
			...
			gasEstimator2 := monacoin.NewGasEstimator(client, 10, pack.NewU256FromUint64(fallback2))
			...
			gasEstimator3 := monacoin.NewGasEstimator(client, 100, pack.NewU256FromUint64(fallback3))
